...
{#Begin Function
  ...
  #Check if there are values of no use
  y = y[y != 0]
  x = x[x != 0]
  ...
  h = center[1] + (radius*cos(theta))
  k = center[2] + (radius*sin(theta))
  plot(h,k, type = 'l', xlim = c(0,max(p[,1])), ylim = c(0,max(p[,2])), main = "Graph of Circle and 
       Provided Points", ylab = "Y-Axis", xlab = "X-Axis")
  #Plot of matrix of points
  points(p[,1], p[,2], col = "blue")
  #Points inside circle are red
  points(x,y, col = "red")
...
{#Begin Function
  ...
  if((day > 31 | day < 1)){stop("Day must fall between 1-12")}
